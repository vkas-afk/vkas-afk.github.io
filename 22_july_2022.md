## Arbitrary File Delete in BlueStacks (CVE-2021-XXXX)
This post will detail how I discovered and exploited CVE-2021-XXXX to gain a arbitrary file delete as root in BlueStacks. 

The vulnerability was that the application had the entitlement 'get-task-allow' allowing other applications to inject threads into the running process, which allows an attacker to make arbitrary XPC calls to the privledged service that the program interacts with because the privledged service authenticates the incoming connection based on the code signature of the binary that is coming from. A thread injection vulnerablity (such as get-task-allow) allows us to pass these checks whilst still running arbitary code.  

To exploit this vulnerablity and gain arbitrary file delete we are going to write Objective-C runtime based shellcode payload to interact with the XPC service as the process that we are injecting into has low privledges. This shellcode will call a function using XPC that deletes arbitrary files as root, in the examples used we will delete /tmp/xpc_test.

#### Discovering the Vulnerability 
One of the first things I do when I review a MacOS application is check the entitlements that the application has because that defines how you can interact with the application and what the application can interact with. To do this you run 
```
...
The following two entitlements allow us to inject and run arbitrary libraries that are unsigned into the process, we will use these entitlements to debug the Objective-C runtime code that we write and test our initial Objective-C shellcode.
...
Now that we discovered that we can start remote threads in the process we need to see what we can do with those threads. Because the application runs with low privledges we will look at the privledged service that it interacts with. 

To dump the protocols that the privledged helper service (com.BlueStacks.AppPlayer.bstservice_helper) supports we use class-dump, filtering through the class-dump output we will find the below protocol definition for HelperIpcProtocol that the Bluestacks application uses modify certain properties of files on the disk. 
```objc
...
@end
```
Looking at the functionality of removeBundleAtPath we can see that it allows an attacker to remove files from anywhere on the host after checking that the binary that is making the XPC request is allowed to make the request. Below is the functionality of the function with the logging moved. 
...
```
Setup the NSAutoreleasePool so that we can use objects th require interact with the NSAutoreleasePool.
```c
...
```
Set the interface on the connection to the be the interface created with the HelperIpcProtocol protocol.
```c
...
```
#### Patching The Code
We compile this code with clang and check that it works by injecting the compiled dylib into the process to make sure that our Objective-C runtime code is correct. Once that is done we need to patch the following pointers at run time for the callback block (simpleblock).
- arg_descriptor_ptr 
...
The data that just needs to read permissions will go into the code section and the data that needs read/write  permissions will go into the data section. 

before we can actually run our thread and call dlopen/dlsym we need add a small bit of code that 'promotes' the mach thread we just created into a pthread that can make all syscalls to do this we are going to modify the code created by Scott Knight(https://gist.github.com/knightsc/45edfc4903a9d2fa9f5905f60b02ce5a)and append our shellcode to the end.
...
Once this done we need to write up a injector to inject code and data into the remote process and then start a thread, using task_for_pid, thread_create_running and the mach_vm_* family of functions.

get a mach_port for the process by looking up the PID and calling task_for_pid()
```c
kern_return_t kr = task_for_pid(current_task(), bluestacksPID, &remoteTask);
```
using that mach_port we will allocate data, code and stack page in the process using mach_vm_allocate(). 
```c
...
```
we then launch or remote thread using thead_create_running().
```c
...
```
We will also dynamically resolve the symbols we need and write a small function to patch the shellcode automatically.
